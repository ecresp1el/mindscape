...
usage() {
  cat <<EOF
Usage: ACCOUNT=... TIME=... CPUS=... MEM=... ./scripts/cellranger/slurm/submit_cellranger_div30.sh
...
Env vars (optional):
  ...
  JOB_NAME    Job name (default: cellranger_multi_div30)
  LOG_DIR     Logs directory (default: $TEST_DIR/logs or ./logs)

Required runtime config for the job body comes from:
  scripts/cellranger/scripts/config_div30.sh (edit or export env vars)

Examples:
  TEST_DIR=/scratch/$USER/mindscape \
  TURBO_CONFIG_SOURCE=/path/to/DIV30/multi-config.csv \
  ...
  ./scripts/cellranger/slurm/submit_cellranger_div30.sh
EOF
}
...
JOB_SCRIPT="$SUBMIT_DIR/job_cellranger_div30.sh"
[[ -f "$JOB_SCRIPT" ]] || { echo "❌ Missing job script: $JOB_SCRIPT" >&2; exit 1; }

JOB_NAME=${JOB_NAME:-cellranger_multi_div30}
