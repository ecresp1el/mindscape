#!/usr/bin/env bash
#
# scripts/cellranger/scripts/config_div30.sh
# Parameterized configuration for DIV30 Cell Ranger pipeline.
#
# Usage:
...
#     DRY_RUN=1 bash scripts/cellranger/scripts/create_project_cellranger_div30.sh
#
#   Slurm submission (actual run):
#     TIME=48:00:00 CPUS=32 MEM=128G ACCOUNT=parent0 \
#     scripts/cellranger/div30/drive_div30.sh slurm
#
# Notes:
...
TEST_DIR=${TEST_DIR:-/nfs/turbo/umms-parent/${USER:-unknown}/mindscape_div30/${RUNSTAMP}} # Test directory per run ans per user

# Source multi-config CSV for DIV30 (required)
# Default: prior DIV30 config used in cellranger pipeline notes
# Note: path contains spaces; keep quotes when overriding.
TURBO_CONFIG_SOURCE=${TURBO_CONFIG_SOURCE:-"/nfs/turbo/umms-parent/Accessible_multi-config_csvs/Carmen_Miranda_scRNAseq /30 Day results/fastqs_9583-MW/multi-config.csv"} # For running cellranger
...
# Accessibility enforcement (DIV30 default)
...
OUTPUT_ID=${OUTPUT_ID:-div30-reanalysis}
