...
# ─────────────────────────────────────────────────────────────────────────────
# scripts/cellranger/scripts/create_project_cellranger_div30.sh
#
# - Sources parameterized config_div30.sh
...
CONFIG_FILE="$SCRIPT_DIR/config_div30.sh" # Creates config file path
...
if [[ "${ENFORCE_ACCESSIBLE:-1}" == "1" ]]; then
  if [[ "$TURBO_CONFIG_SOURCE" != *"/Accessible_multi-config_csvs/"* ]]; then
    echo "❌ TURBO_CONFIG_SOURCE must come from an Accessible_multi-config_csvs path for DIV30." >&2
