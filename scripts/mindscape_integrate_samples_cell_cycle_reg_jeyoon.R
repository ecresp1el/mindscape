...
# Save outputs
# ------------------------------------------------------------------------------
write.csv(as.data.frame(Idents(merged)), file = file.path(output_dir, paste0("integrated_cluster_ids.csv")))
cat("✅ Cluster IDs saved\n")

png(file.path(output_dir, "integrated_umap_cell_cycle_reg_jeyoon.png"), width = 800, height = 600)
