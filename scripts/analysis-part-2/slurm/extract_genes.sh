...
# ==============================================================================
# SLURM Job Script for MindScape - Extract Multiple Genes with Cell Identity (Day 90)
# ==============================================================================

#SBATCH --job-name=extract_genes_day90
#SBATCH --account=parent0
#SBATCH --output=/nfs/turbo/umms-parent/Manny_test/ventral_sosrs_output/logs/extract_genes_day90_%j.out
#SBATCH --error=/nfs/turbo/umms-parent/Manny_test/ventral_sosrs_output/logs/extract_genes_day90_%j.err
...
# Input CSV (DEGs_all_clusters.csv for Day 90)
export DEG_INPUT_CSV="$BASE_DIR/ventral_sosrs_output_integrated_analysis_cell_cycle_reg_jeyoon_DEGs/DEGs_all_clusters/DEGs_all_clusters.csv"

# Output directory for extracted genes
export DEG_OUTPUT="$BASE_DIR/ventral_sosrs_output_integrated_analysis_cell_cycle_reg_jeyoon_gene_extracts/gene_extracts"
mkdir -p "$DEG_OUTPUT"

# ------------------------------------------------------------------------------
# Target genes + identities (order matters: gene[i] → identity[i])
# ------------------------------------------------------------------------------
# ⚠️ You can reuse or customize the gene/identity lists for Day 90 below.
# For now, this version reuses the same lists from Day 30 for consistency.
# ------------------------------------------------------------------------------
...
echo "📊 Extracting entries for multiple genes (Day 90)..."
Rscript "$SCRIPT_PATH"
EXIT_CODE=$?
...
echo "✅ Extraction complete (Day 90)"
